...
export default function AiVoiceGenerator() {
    const apiKey = "AIzaSyDVdBtghShZV7FehlGPwmzAq-EPPISQwq0"; // Masukkan API Key Gemini Kamu di Sini jika diperlukan
        ...
        try {
            ...
            if (!isRebakeOnly) {
                ...
                if (effectiveMode === 'optimized') {
                    ...
                    const refineResp = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`, {
                        method: "POST", headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ contents: [{ parts: [{ text: logicPrompt }] }] })
                    });
                    if (!refineResp.ok) throw new Error("AI Director Error");
                ...
                if (!ttsResp.ok) throw new Error("TTS Generation Error");
                const ttsData = await ttsResp.json();
